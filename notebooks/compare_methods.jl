...
# ╔═╡ c347e5d8-7da2-4aad-aa56-cec1cd6ee6f7
using SpecialFunctions

# ╔═╡ b81c4dba-caf2-4293-93be-134ae7194877
using Luxor
...
method for solving the scattering through one periodic sheet with a 1D unit cell.
"
...
kᵢ = 10.0 # incident wavenumber = ω/c
...
k = kᵢ / 1.1 # wavenumber that attains the metasurface design goals
...
β⃗ = @. sqrt(Complex(kᵢ^2 - k⃗^2))

# ╔═╡ badfeed7-aabf-44ae-8a97-ab5ca797bf76
i = findfirst(k⃗ .- αᵢ .< eps()) # mode index of incident wave

# ╔═╡ d1b49c5e-e9a2-4b44-90e9-622fab40c740
@assert i == findfirst(real.(β⃗ .- βᵢ) .< eps())
...
uInc(x,y)= exp(im*αᵢ*x+im*βᵢ*y)  # incident planewave
...
NC = 6      # Number of unit cells considered in the windowed sum (A = NC*L)

# ╔═╡ f8bb8da7-20d0-43da-846e-5130e7b38edc
mat = matricesSkeleton(1,Γ,kᵢ,L,αᵢ,NC,method); # Construction of the Nystrom matrices
...
f1(x) = -2*im*kᵢ*η(x)*exp(im*αᵢ*x)

# ╔═╡ cf4f4bf4-8a47-401a-b569-905c9b3cd9fd
f2(x) = -2*im*βᵢ*exp(im*αᵢ*x)

# ╔═╡ b08df8b1-de6f-4b71-a650-48e483d91a3e
# Solving the boundary integral equations
begin
A⁺ = -0.5*I+im*kᵢ*Diagonal(η.(x1))*mat.S  # System matrix for Ω⁺
b⁺ = f1.(x1) # boundary data for Ω⁺
ψ⁺ = A⁺\b⁺

A⁻ = -0.5*I+im*kᵢ*Diagonal(μ.(x1))*mat.S # System matrix for Ω⁻
...
pot⁺ = potentialsSkeleton(Ω⁺.pts,Γ,kᵢ,L,αᵢ,NC);
...
begin
	...
	heatmap(x, y, part.(fields), xguide="x", yguide="y", aspect_ratio=:equal,color=:RdBu,)
end
...
phase = -5*dx # this is a fudge factor to reduce the apparent far-field error # try -5dx
