...
"""
function smat_star(A::T, B::T) where T<:BlockMatrix
    ...
    return mortar(reshape([
        A[Block(1, 1)] + A[Block(1, 2)] * I_BA * B[Block(1, 1)] * A[Block(2, 1)],
        B[Block(2, 1)] * I_AB * A[Block(2, 1)],
        A[Block(1, 2)] * I_BA * B[Block(1, 2)],
        B[Block(2, 2)] + B[Block(2, 1)] * I_AB * A[Block(2, 2)] * B[Block(1, 2)],
    ], 2, 2))
end

"""
    rred_star(A::T, B::T) where T<:BlockMatrix
